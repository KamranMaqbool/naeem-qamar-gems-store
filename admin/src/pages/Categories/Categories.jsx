...
export default function Categories() {
  ...
  const submit = async (event) => {
    ...
    } catch (submitError) { setError(submitError.message || 'Unable to save category.'); setFieldErrors(submitError.details || {}); }
  ...
  return (
    <div className="mx-auto w-full max-w-5xl">
      ...
      {error && <div className="mb-5 rounded-lg border border-error/30 bg-error-bg px-4 py-3 text-sm text-error-text" role="alert"><p className="font-semibold">Please correct the highlighted fields.</p><p className="mt-1">{error}</p></div>}
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        ...
        <form onSubmit={submit} className="card h-fit space-y-4 p-6">
          <h2 className="text-xl font-semibold text-on-surface">{editingId ? 'Edit category' : 'Add category'}</h2>
          <label className="block text-sm font-medium">Name
            <input required value={form.name} onChange={(event) => { setForm({ ...form, name: event.target.value }); setFieldErrors((current) => ({ ...current, name: undefined })); }} className={`mt-2 w-full rounded-lg border px-3 py-2 ${fieldErrors.name ? 'border-error' : 'border-outline-variant'}`} />
            <FieldMessage messages={fieldErrors.name} />
          </label>
          <label className="block text-sm font-medium">Slug
            <input value={form.slug} onChange={(event) => { setForm({ ...form, slug: event.target.value }); setFieldErrors((current) => ({ ...current, slug: undefined })); }} className={`mt-2 w-full rounded-lg border px-3 py-2 ${fieldErrors.slug ? 'border-error' : 'border-outline-variant'}`} />
            <FieldMessage messages={fieldErrors.slug} />
          </label>
          <label className="block text-sm font-medium">Description
            <textarea value={form.description} onChange={(event) => { setForm({ ...form, description: event.target.value }); setFieldErrors((current) => ({ ...current, description: undefined })); }} className={`mt-2 w-full rounded-lg border px-3 py-2 ${fieldErrors.description ? 'border-error' : 'border-outline-variant'}`} rows={4} />
            <span className="mt-1 block text-right text-xs font-normal text-on-surface-variant" aria-live="polite">{countWords(form.description)} words</span>
            <FieldMessage messages={fieldErrors.description} />
          </label>
          <button disabled={saving} className="w-full rounded-lg bg-primary px-4 py-3 font-semibold text-on-primary disabled:opacity-60">{saving ? 'Saving…' : editingId ? 'Update category' : 'Create category'}</button>
          {editingId && <button type="button" onClick={() => { setEditingId(null); setForm(emptyForm); setFieldErrors({}); setError(''); }} className="w-full py-2 text-sm text-on-surface-variant">Cancel</button>}
        </form>
      </div>
